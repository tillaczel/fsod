...
class FastRCNNOutputs(object):
    ...
    def _log_accuracy(self):
        ...
        num_fg = torch.nonzero(fg_inds).numel()  # fg_inds.nonzero().numel()
        fg_gt_classes = self.gt_classes[fg_inds]
        fg_pred_classes = pred_classes[fg_inds]

        num_false_negative = (
            torch.nonzero(fg_pred_classes == bg_class_ind).numel()  # (fg_pred_classes == bg_class_ind).nonzero().numel()
        )
...
class CLIPClassifier(nn.Module):
    def __init__(self, cfg, input_size, num_classes):
        ...
        dataset_dicts = [MetadataCatalog.get(dataset_name) for dataset_name in (cfg.DATASETS.TRAIN+cfg.DATASETS.TEST)]
        classes = set()
        for dd in dataset_dicts:
            classes.update(dd.thing_classes)
        assert num_classes == len(classes)

        model, preprocess = clip.load("ViT-B/32", device=cfg.MODEL.DEVICE)
        promts = ['a photo of a {} in a scene'.format(c) for c in classes]
        text = clip.tokenize(promts).to(cfg.MODEL.DEVICE)
        with torch.no_grad():
            text_features = model.encode_text(text)
        del model, preprocess, promts, text

        embedding_dim = text_features.shape[1]
        self.projector = nn.Linear(input_size, embedding_dim)
        self.text_features = torch.tensor(text_features.clone().detach())
        self.weight = Parameter(torch.empty(1, embedding_dim))
        self.bias = Parameter(torch.empty(1))  # this is needed for the model surgery to work, TODO: fix model surgery so this is not needed
        self.reset_parameters()

    def reset_parameters(self) -> None:
        init.kaiming_uniform_(self.weight, a=math.sqrt(5))
        nn.init.constant_(self.bias, 0)  # same reason

    def forward(self, input: Tensor) -> Tensor:
        x = F.relu(self.projector(input))
        x = F.linear(x, torch.cat((self.text_features, self.weight), dim=0))
        return x
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        classifier = cfg.MODEL.ROI_HEADS.CLASSIFIER
        if classifier == "linear":
            ...
            self.cls_score = CLIPClassifier(cfg, input_size, num_classes)
        else:
